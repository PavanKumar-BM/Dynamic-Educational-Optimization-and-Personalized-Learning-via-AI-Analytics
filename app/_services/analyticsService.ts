 ...
 */
export async function findCourseRowId(
  ...
  try {
    const result = await db
      .select()
      .from(CourseList)
      .where(eq(CourseList.courseId, courseExternalId));
 ...
 */
export async function findChapterRowId(
  courseExternalId: string,
  chapterId: number
): Promise<number | null> {
  try {
    const result = await db
      ...
      .where(
        and(
          eq(CourseChapters.courseId, courseExternalId),
...
export async function startStudySession(
  ...
  if (typeof chapterRowId !== "number")
    throw new Error("chapterRowId is required by schema");
  try {
    const startTime = convertDateToUnixTimestamp(new Date());
    const [session] = await db
      .insert(UserStudySessions)
      .values({
        userId,
        courseRowId,
        chapterRowId,
...
export async function endStudySession(sessionId: number): Promise<void> {
  try {
    ...
    const session = await db
      ...
      .where(eq(UserStudySessions.sessionId, sessionId));
    if (!session[0]) return;
    const duration = endTime - session[0].startTime;
    await db
      .update(UserStudySessions)
...
export async function getActiveSession(
  ...
  courseRowId: number,
  chapterRowId: number
): Promise<UserStudySessionType | null> {
  try {
    const whereClause = and(
      eq(UserStudySessions.userId, userId),
      eq(UserStudySessions.courseRowId, courseRowId),
      eq(UserStudySessions.chapterRowId, chapterRowId)
    );
    ...
    return {
      ...
      endTime: sessions[0].endTime
        ? convertUnixTimestampToDate(sessions[0].endTime)
        : undefined,
...
export async function updateChapterProgress(
  ...
  courseRowId: number,
  chapterRowId: number,
  progress: Partial<ChapterProgressType>
): Promise<void> {
  try {
    const updateObj: any = {};
    if (typeof progress.isCompleted === "boolean")
      updateObj.isCompleted = progress.isCompleted;
    if (typeof progress.timeSpent === "number")
      updateObj.timeSpent = progress.timeSpent;
    if (
      typeof progress.completionDate === "object" &&
      progress.completionDate instanceof Date
    )
      updateObj.completionDate = convertDateToUnixTimestamp(
        progress.completionDate
      );
    if (typeof progress.progressPercentage === "number")
      updateObj.progressPercentage = progress.progressPercentage;
    await db
      ...
      .where(
        and(
          ...
          eq(ChapterProgress.courseRowId, courseRowId),
          eq(ChapterProgress.chapterRowId, chapterRowId)
        )
      );
  } catch (err) {
...
export async function updateCourseProgress(
  userId: string,
  courseRowId: number,
  progress: Partial<CourseProgressType>
): Promise<void> {
  try {
    const updateObj: any = {};
    if (typeof progress.totalTimeSpent === "number")
      updateObj.totalTimeSpent = progress.totalTimeSpent;
    if (typeof progress.completionPercentage === "number")
      updateObj.completionPercentage = progress.completionPercentage;
    if (typeof progress.chaptersCompleted === "number")
      updateObj.chaptersCompleted = progress.chaptersCompleted;
    if (
      typeof progress.lastAccessedDate === "object" &&
      progress.lastAccessedDate instanceof Date
    )
      updateObj.lastAccessedDate = convertDateToUnixTimestamp(
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(CourseProgress.courseRowId, courseRowId)
        )
      );
  } catch (err) {
...
export async function getChapterProgress(
  ...
  courseRowId: number,
  chapterRowId: number
): Promise<ChapterProgressType | null> {
  try {
    const result = await db
      .select()
    ...
    return {
      ...
      completionDate: result[0].completionDate
        ? convertUnixTimestampToDate(result[0].completionDate)
        : undefined,
...
export async function getCourseProgress(
  ...
  try {
    ...
    return {
      ...
      lastAccessedDate: result[0].lastAccessedDate
        ? convertUnixTimestampToDate(result[0].lastAccessedDate)
        : undefined,
      isCompleted: result[0].isCompleted,
    };
  } catch (err) {
 ...
 */
export async function getUserStudySessionsByRange(
  userId: string,
  range?: DateRangeFilter
): Promise<UserStudySessionType[]> {
  try {
    ...
    if (range) {
      whereClauses.push(
        and(
          ...
          eq(
            UserStudySessions.startTime,
            convertDateToUnixTimestamp(range.start)
          ),
 ...
 */
export async function getUserStudyTimeByDateRange(
  userId: string,
  range: DateRangeFilter
): Promise<StudyTimeChartData[]> {
  try {
    ...
    return Object.entries(grouped).map(([date, duration]) => ({
      date,
      duration,
    }));
  } catch (err) {
 ...
 */
export async function getUserChapterProgressData(
  userId: string
  ...
  try {
    ...
    return chapters.map((c) => ({
      chapter: `Chapter ${c.chapterRowId}`,
 ...
 */
export async function calculateTotalStudyTime(
  userId: string,
  range: DateRangeFilter
): Promise<number> {
  try {
 ...
 */
export async function calculateAverageProgress(
  userId: string
): Promise<number> {
  try {
 ...
 */
export async function calculateCompletionRates(
  userId: string
...
export async function calculateStudyStreak(
  userId: string
): Promise<StudyStreakData> {
  try {
    const sessions = await getUserStudySessionsByRange(userId);
    const dates = Array.from(
      new Set(sessions.map((s) => s.startTime.toISOString().slice(0, 10)))
    )
      .sort()
      .reverse();
    let streak = 0;
 ...
 */
export function prepareStudyTimeChartData(
 ...
 */
export function prepareCourseDistributionData(
  data: CourseAnalytics[]
): DistributionChartData[] {
  return data.map((c) => ({
    name: `Course ${c.courseRowId}`,
    value: c.chaptersCompleted,
  }));
}
 ...
 */
export function preparePerformanceMetrics(
  data: CourseAnalytics[]
): PerformanceMetricData[] {
  return [
    {
      label: "Total Study Time",
      value: data.reduce((sum, c) => sum + (c.totalTimeSpent || 0), 0),
      icon: null,
    },
    {
      label: "Avg. Progress",
      value: data.length
        ? data.reduce((sum, c) => sum + (c.completionPercentage || 0), 0) /
          data.length
        : 0,
      icon: null,
    },
    { label: "Courses", value: data.length, icon: null },
    {
      ...
      value: data.reduce((sum, c) => sum + (c.chaptersCompleted || 0), 0),
      icon: null,
    },
  ];
}
  ...
  return [
    {
      ...
      description: "You have a strong study streak!",
      type: "strength",
    },
    {
      title: "Opportunity",
      description: "Try to complete more chapters for higher progress.",
      type: "opportunity",
    },
  ];
}
...
export async function detectStudyPatterns(userId: string): Promise<string[]> {
  ...
  return [
    "You study most on weekends.",
    "Your average session duration is increasing.",
  ];
}
  ...
  if (range) {
    ...
    clauses.push(
      and(
        ...
        lte(
          or(isNull(UserStudySessions.endTime), UserStudySessions.endTime),
          end
        )
      )
    );
