...
export function useStudySession(courseId: string, chapterId?: number) {
  ...
  useEffect(() => {
    ...
    async function startSession() {
      try {
        ...
        const chapterRowId = chapterId ? await findChapterRowId(courseId, chapterId) : undefined;
        if (!user?.id || !courseRowId) return;
        const sessionId = await startStudySession(user.id, courseRowId, chapterRowId);
  ...
  useInterval(() => {
    ...
    updateSessionDuration(sessionIdRef.current, now - startTimeRef.current).catch(() => {});
  }, 30000);

  useEffect(() => {
    return () => {
      if (sessionIdRef.current) endStudySession(sessionIdRef.current).catch(() => {});
    };
  }, []);
